...
private:
  ...
  void result_callback(
    ...
    // get the desired object from detected list of objects
    // using type and dimensions to filter the object of interest
    RCLCPP_INFO(this->get_logger(), "# ~~~~~~~~~~~ ~~~~~~~~~~~ ~~~~~~~~~~~ #");
    RCLCPP_INFO(this->get_logger(),
                "Result: Filtered Object of Interest Position:");
    for (auto object : action_result.result->objects) {
      if ((object.object.primitives[0].type == 1) &&
          (object.object.primitives[0].dimensions[0] < 0.050) &&
          (object.object.primitives[0].dimensions[1] < 0.050) &&
          (object.object.primitives[0].dimensions[2] < 0.100)) {
        RCLCPP_INFO(this->get_logger(), "Type: %d",
                    object.object.primitives[0].type);
        RCLCPP_INFO(this->get_logger(),
                    "DimX: %+0.3f, DimY: %+0.3f, DimZ: %+0.3f",
                    object.object.primitives[0].dimensions[0],
                    object.object.primitives[0].dimensions[1],
                    object.object.primitives[0].dimensions[2]);
        RCLCPP_INFO(this->get_logger(),
                    "PosX: %+0.3f, PosY: %+0.3f, PosZ: %+0.3f",
                    action_result.result->objects[0]
